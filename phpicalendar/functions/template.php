...
//			$this->page = preg_replace('!<\!-- switch display_download on -->(.*)<\!-- switch display_download off -->!is', '', $this->page);
			$this->page = preg_replace('!<\!-- switch display_download on -->(.*?)<\!-- switch display_download off -->!is', '', $this->page);	// ungreedify - Rik, 2007-03-18
		}
	}
...
//			$this->page 	= preg_replace('!<\!-- switch logged_in2 on -->(.*)<\!-- switch logged_in2 off -->!is', '', $this->page);
			$this->page 	= preg_replace('!<\!-- switch logged_in on -->(.*?)<\!-- switch logged_in off -->!is', '', $this->page);	// ungreedified - Rik 2007-03-18
			$this->page 	= preg_replace('!<\!-- switch logged_in2 on -->(.*?)<\!-- switch logged_in2 off -->!is', '', $this->page);	// ungreedified - Rik 2007-03-18
		}
	
	}
	
	function draw_print($template_p) {
		global $template, $getdate, $cal, $master_array, $daysofweek_lang, $week_start_day, $printview, $dateFormat_day, $timeFormat, $week_start, $week_end, $lang, $this_year, $this_month;
...
//		preg_match("!<\!-- switch some_events on -->(.*)<\!-- loop events on -->!is", $this->page, $match3);
		preg_match("!<\!-- loop events on -->(.*?)<\!-- loop events off -->!is", $this->page, $match1);	// ungreedified - Rik 2007-03-18
		preg_match("!<\!-- switch some_events on -->(.*?)<\!-- loop events on -->!is", $this->page, $match3);	// ungreedified - Rik 2007-03-18
		...
		foreach($master_array as $key => $val) {
			...
			if (((@$regs[1] == $parse_month) && ($printview == 'month')) || (($key == $getdate) && ($printview == 'day')) || ((($key >= $week_start) && ($key <= $week_end)) && ($printview == 'week' || $printview == 'agenda')) || ((substr(@$regs[1],0,4) == $parse_year) && ($printview == 'year'))) {
				$events_week++;
				...
				foreach ($val as $cal_time => $new_val) {
					...
					if ($new_val2['event_text']) {	
						...
						$event_text = openevent($m_start, $cal_time, $new_key2, $new_val2, 0, 15, 'psf');
						...
						if (isset($new_val2['display_end'])) $event_end = $new_val2['display_end'];
							if (!$new_val2['event_start']) { 
								...
								//$event_start	= str_replace(':00', '', $event_start); // Rik, 2006-7-03
							}
						}
						
						if ($description == '') {
//							$events_tmp = preg_replace('!<\!-- switch description_events on -->(.*)<\!-- switch description_events off -->!is', '', $events_tmp);
							$events_tmp = preg_replace('!<\!-- switch description_events on -->(.*?)<\!-- switch description_events off -->!is', '', $events_tmp); // ungreedified - Rik 2007-03-18
						}

						if ($location == '') {
//							$events_tmp = preg_replace('!<\!-- switch location_events on -->(.*)<\!-- switch location_events off -->!is', '', $events_tmp);
							$events_tmp = preg_replace('!<\!-- switch location_events on -->(.*?)<\!-- switch location_events off -->!is', '', $events_tmp); // ungreedified - Rik 2007-03-18
						}
...
//			$this->page = preg_replace('!<\!-- switch some_events on -->(.*)<\!-- switch some_events off -->!is', '', $this->page);
			$this->page = preg_replace('!<\!-- switch some_events on -->(.*?)<\!-- switch some_events off -->!is', '', $this->page);	// ungreedified - Rik 2007-03-18
		} else {
...
//			$this->page = preg_replace('!<\!-- switch no_events on -->(.*)<\!-- switch no_events off -->!is', '', $this->page);
			$this->page = preg_replace('!<\!-- switch some_events on -->(.*?)<\!-- switch some_events off -->!is', $final, $this->page);	// ungreedified - Rik 2007-03-18
			$this->page = preg_replace('!<\!-- switch no_events on -->(.*?)<\!-- switch no_events off -->!is', '', $this->page);	// ungreedified - Rik 2007-03-18
		}
	}	
...
//		preg_match("!<\!-- switch exceptions on -->(.*)<\!-- switch exceptions off -->!is", $this->page, $match3);
		preg_match("!<\!-- switch results on -->(.*?)<\!-- switch results off -->!is", $this->page, $match1);	// ungreedified - Rik 2007-03-18
		preg_match("!<\!-- switch recur on -->(.*?)<\!-- loop recur off -->!is", $this->page, $match2);	// ungreedified - Rik 2007-03-18
		preg_match("!<\!-- switch exceptions on -->(.*?)<\!-- switch exceptions off -->!is", $this->page, $match3);	// ungreedified - Rik 2007-03-18
		$loop_event		= trim($match1[1]);
...
//					$events_tmp = preg_replace('!<\!-- switch exceptions on -->(.*)<\!-- switch exceptions off -->!is', '', $events_tmp);
					$events_tmp = preg_replace('!<\!-- switch exceptions on -->(.*?)<\!-- switch exceptions off -->!is', '', $events_tmp);	// ungreedified - Rik 2007-03-18
				}else{
						$some_exceptions = "";
...
//								$except_tmp = preg_replace('!<\!-- switch except_recur on -->(.*)<\!-- switch except_recur off -->!is', '', $except_tmp);
								$except_tmp = preg_replace('!<\!-- switch except_recur on -->(.*?)<\!-- switch except_recur off -->!is', '', $except_tmp);	// ungreedified - Rik 2007-03-18
							}else{
...
//								$except_tmp = preg_replace('!<\!-- switch except_description on -->(.*?)<\!-- switch except_description off -->!is', '', $except_tmp);
								$except_tmp = preg_replace('!<\!-- switch except_description on -->(.*)<\!-- switch except_description off -->!is', '', $except_tmp);	// ungreedified - Rik 2007-03-18
							}else{
...
//						$events_tmp = preg_replace('!<\!-- switch exceptions on -->(.*)<\!-- switch exceptions off -->!is', $some_exceptions,$events_tmp );
						$events_tmp = preg_replace('!<\!-- switch exceptions on -->(.*?)<\!-- switch exceptions off -->!is', $some_exceptions,$events_tmp );	// ungreedified - Rik 2007-03-18
...
//					$events_tmp = preg_replace('!<\!-- switch recur on -->(.*)<\!-- switch recur off -->!is', '', $events_tmp);
					$events_tmp = preg_replace('!<\!-- switch recur on -->(.*?)<\!-- switch recur off -->!is', '', $events_tmp);	// ungreedified - Rik 2007-03-18
				}else{
...
//			$this->page = preg_replace('!<\!-- switch results on -->(.*)<\!-- switch results off -->!is', '', $this->page);
			$this->page = preg_replace('!<\!-- switch results on -->(.*?)<\!-- switch results off -->!is', '', $this->page);	// ungreedified - Rik 2007-03-18
		} else {
...
//			$this->page = preg_replace('!<\!-- switch no_results on -->(.*)<\!-- switch no_results off -->!is', '', $this->page);
			$this->page = preg_replace('!<\!-- switch results on -->(.*?)<\!-- switch results off -->!is', $final, $this->page);	// ungreedified - Rik 2007-03-18
			$this->page = preg_replace('!<\!-- switch no_results on -->(.*?)<\!-- switch no_results off -->!is', '', $this->page);	// ungreedified - Rik 2007-03-18
	...
	function draw_week($template_p) {
		global $unique_colors, $start_week_time, $template, $getdate, $cal, $master_array, $daysofweek_lang, $week_start_day, $dateFormat_week_list, $current_view, $day_array, $timeFormat, $gridLength, $timeFormat_small, $day_start, $week_length, $today;	// $today added by Rik, 2006/11/30
...
//		preg_match("!<\!-- loop allday off -->(.*)<\!-- loop alldaysofweek off -->!is", $this->page, $match3);
		preg_match("!<\!-- loop allday on -->(.*?)<\!-- loop allday off -->!is", $this->page, $match1);	// ungreedified - Rik 2007-03-18
		preg_match("!<\!-- loop alldaysofweek on -->(.*?)<\!-- loop allday on -->!is", $this->page, $match2);	// ungreedified - Rik 2007-03-18
		preg_match("!<\!-- loop allday off -->(.*?)<\!-- loop alldaysofweek off -->!is", $this->page, $match3);	// ungreedified - Rik 2007-03-18
			...
			if (is_array($master_array[$get_date]['-1']) && !empty($master_array[$get_date]['-1']) ) {
				foreach ($master_array[$get_date]['-1'] as $uid => $allday) {
					...
					$event_calno	= (($event_calno - 1) % $unique_colors) + 1;
 					$event 			= openevent($get_date, -1, $uid, $allday, 1, 15, 'psf');	// Rik 2006/12/02: replaced 11 with 15
...
//		$this->page = preg_replace('!<\!-- loop alldaysofweek on -->(.*)<\!-- loop alldaysofweek off -->!is', $weekreplace, $this->page);
		$this->page = preg_replace('!<\!-- loop alldaysofweek on -->(.*?)<\!-- loop alldaysofweek off -->!is', $weekreplace, $this->page);	// ungreedified - Rik 2007-03-18
		
		// Replaces the daysofweek
//		preg_match("!<\!-- loop daysofweek on -->(.*)<\!-- loop daysofweek off -->!is", $this->page, $match1);
		preg_match("!<\!-- loop daysofweek on -->(.*?)<\!-- loop daysofweek off -->!is", $this->page, $match1);	// ungreedified - Rik 2007-03-18
		...
		for ($i=0; $i<$week_length; $i++) {
			...
			// next line replaced by Rik, 2006/11/30
			// if ($daylink == $getdate) {
			if ($daylink == $today) {
				$row1 = 'rowToday';
				$row2 = 'rowOn';
...
//		$this->page = preg_replace('!<\!-- loop daysofweek on -->(.*)<\!-- loop daysofweek off -->!is', $weekday_loop, $this->page);
		$this->page = preg_replace('!<\!-- loop daysofweek on -->(.*?)<\!-- loop daysofweek off -->!is', $weekday_loop, $this->page);	// ungreedified - Rik 2007-03-18
		
		// Build the body
//		preg_match("!<\!-- loop row on -->(.*)<\!-- loop row off -->!is", $this->page, $match2);
//		preg_match("!<\!-- loop event on -->(.*)<\!-- loop event off -->!is", $this->page, $match3);
		preg_match("!<\!-- loop row on -->(.*?)<\!-- loop row off -->!is", $this->page, $match2);	// ungreedified - Rik 2007-03-18
		preg_match("!<\!-- loop event on -->(.*?)<\!-- loop event off -->!is", $this->page, $match3);	// ungreedified - Rik 2007-03-18
...
//		$this->page = preg_replace('!<\!-- loop row on -->(.*)<\!-- loop event off -->!is', $weekdisplay, $this->page);
		$this->page = preg_replace('!<\!-- loop row on -->(.*?)<\!-- loop event off -->!is', $weekdisplay, $this->page);	// ungreedified - Rik 2007-03-18
	}
...
//		$this->page = preg_replace('!<\!-- loop allday on -->(.*)<\!-- loop allday off -->!is', $replace, $this->page);
		$this->page = preg_replace('!<\!-- loop allday on -->(.*?)<\!-- loop allday off -->!is', $replace, $this->page);	// ungreedified - Rik 2007-03-18
...
//		$this->page = preg_replace('!<\!-- loop row on -->(.*)<\!-- loop event off -->!is', $daydisplay, $this->page);
		$this->page = preg_replace('!<\!-- loop row on -->(.*?)<\!-- loop event off -->!is', $daydisplay, $this->page);	// ungreedified - Rik 2007-03-18
...
//		preg_match("!<\!-- switch t_event on -->(.*)<\!-- switch t_event off -->!is", $this->page, $match2);
		preg_match("!<\!-- switch t_allday on -->(.*?)<\!-- switch t_allday off -->!is", $this->page, $match1);	// ungreedified - Rik 2007-03-18
		preg_match("!<\!-- switch t_event on -->(.*?)<\!-- switch t_event off -->!is", $this->page, $match2);	// ungreedified - Rik 2007-03-18
...
//			$this->page = preg_replace('!<\!-- switch t_event on -->(.*)<\!-- switch t_event off -->!is', $replace_e, $this->page);		
			$this->page = preg_replace('!<\!-- switch t_allday on -->(.*?)<\!-- switch t_allday off -->!is', $replace_ad, $this->page);	// ungreedified - Rik 2007-03-18
			$this->page = preg_replace('!<\!-- switch t_event on -->(.*?)<\!-- switch t_event off -->!is', $replace_e, $this->page);	// ungreedified - Rik 2007-03-18

		} else {

//			$this->page = preg_replace('!<\!-- switch tomorrows_events on -->(.*)<\!-- switch tomorrows_events off -->!is', '', $this->page);
			$this->page = preg_replace('!<\!-- switch tomorrows_events on -->(.*?)<\!-- switch tomorrows_events off -->!is', '', $this->page);	// ungreedified - Rik 2007-03-18
...
//		preg_match("!<\!-- switch show_normal on -->(.*)<\!-- switch show_normal off -->!is", $this->page, $match3);
		preg_match("!<\!-- switch show_completed on -->(.*?)<\!-- switch show_completed off -->!is", $this->page, $match1);	// ungreedified - Rik 2007-03-18
		preg_match("!<\!-- switch show_important on -->(.*?)<\!-- switch show_important off -->!is", $this->page, $match2);	// ungreedified - Rik 2007-03-18
		preg_match("!<\!-- switch show_normal on -->(.*?)<\!-- switch show_normal off -->!is", $this->page, $match3);	// ungreedified - Rik 2007-03-18
...
//			$this->page = preg_replace('!<\!-- switch vtodo on -->(.*)<\!-- switch vtodo off -->!is', '', $this->page);
			$this->page = preg_replace('!<\!-- switch vtodo on -->(.*?)<\!-- switch vtodo off -->!is', '', $this->page);	// ungreedified - Rik 2007-03-18
		}
...
//			$this->page = preg_replace('!<\!-- switch show_completed on -->(.*)<\!-- switch show_normal off -->!is', $nugget2, $this->page);
			$this->page = preg_replace('!<\!-- switch show_completed on -->(.*?)<\!-- switch show_normal off -->!is', $nugget2, $this->page);	// ungreedified - Rik 2007-03-18
		}
	}
...
//		$return = preg_replace('!<\!-- loop monthweeks on -->(.*)<\!-- loop monthweeks off -->!is', $middle, $return);
		$return = preg_replace('!<\!-- loop monthweeks on -->(.*?)<\!-- loop monthweeks off -->!is', $middle, $return);	// ungreedified - Rik 2007-03-18
...
//		$this->page = preg_replace('!<\!-- switch showbottom on -->(.*)<\!-- switch showbottom off -->!is','', $this->page);
		$this->page = preg_replace('!<\!-- switch showbottom on -->(.*?)<\!-- switch showbottom off -->!is','', $this->page);	// ungreedified - Rik 2007-03-18
	}

	function nosearch() {
//		$this->page = preg_replace('!<\!-- switch show_search on -->(.*)<\!-- switch show_search off -->!is','', $this->page);
		$this->page = preg_replace('!<\!-- switch show_search on -->(.*?)<\!-- switch show_search off -->!is','', $this->page);	// ungreedified - Rik 2007-03-18
	}
...
//		$this->page = preg_replace('!<\!-- loop showbottomevents_odd on -->(.*)<\!-- loop showbottomevents_even off -->!is', $middle, $this->page);
		$this->page = preg_replace('!<\!-- loop showbottomevents_odd on -->(.*?)<\!-- loop showbottomevents_even off -->!is', $middle, $this->page);	// ungreedified - Rik 2007-03-18
...
//					$this->page = preg_replace('!<\!-- switch ' . $tag . ' on -->(.*)<\!-- switch ' . $tag . ' off -->!is', '', $this->page);
					$this->page = preg_replace('!<\!-- switch ' . $tag . ' on -->(.*?)<\!-- switch ' . $tag . ' off -->!is', '', $this->page);	// ungreedified - Rik 2007-03-18
				}
...
//			$this->page = preg_replace('!<\!-- switch rss_powered on -->(.*)<\!-- switch rss_powered off -->!is', '', $this->page);
			$this->page = preg_replace('!<\!-- switch rss_powered on -->(.*?)<\!-- switch rss_powered off -->!is', '', $this->page);	// ungreedified - Rik 2007-03-18
		} else {
